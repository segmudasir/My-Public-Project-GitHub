...
namespace My_Project
{
    ...
    {
        float T1, T3, T4, D1, D3, D4, ID, WE, VSD, RV, DS, TS, TS2, RT, Y, M, X, C, Subsea;
    ...
    //////////////////////////////////Depth Calculation Funciton/////////////////////////////////////////////////
    public float CalculateDepth(float time)
    {
        counter = 0;
            while (Time2.Count > 0)
            {
            ...
            if (RT >= Time[0])
            {
                for (int i = 0; i < counter-1; i++)
                {
                    if (RT >= Time[i] && RT < Time[i+1])
                    {
                        ...
                        Y = (M * X) + C;
                        Subsea = WE - Y;
                        textBox16.Text = Subsea.ToString();
    ...
    private void button1_Click(object sender, EventArgs e)
    {
        ...
        private void multiple_CheckedChanged(object sender, EventArgs e)
        {
            ...
            dataGridView2.Columns[0].HeaderText = "Time (ms)";
            dataGridView3.Columns[0].HeaderText = "Depth (MD)";
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                multiTime.Add(float.Parse((dataGridView2.Rows[multicounter].Cells[0].Value).ToString()));
                multicounter++;
            }

            for (int i = 0; i < multicounter; i++)
            {
                CalculateDepth(multiTime[i]);
                multiDepth.Add(Y);
            }
            for (int i = 0; i < multicounter; i++)
            {
                dataGridView3.Rows.Add(multiDepth[i].ToString());
            }
        }
    }
}
